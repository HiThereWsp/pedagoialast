      ...
      <div className="container flex h-16 items-center justify-between">
        ...
        {mobileMenuOpen && <div className="fixed inset-0 z-50 bg-background md:hidden">
            ...
            <nav className="container mt-4 flex flex-col gap-4">
              {links.map(link => <Link key={link.name} to={link.href} className={`p-2 rounded-md text-base ${location.pathname === link.href ? 'bg-muted text-foreground font-medium' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'}`} onClick={() => setMobileMenuOpen(false)}>
                  {link.name}
                </Link>)}
