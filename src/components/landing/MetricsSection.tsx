        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {metrics.map((item, index) => (
              <Card 
                key={index} 
                className={`p-6 bg-gradient-to-br ${item.gradient} backdrop-blur-sm border-none shadow-premium hover:shadow-premium-lg transition-all duration-300 transform hover:scale-105 group`}
              >
                <div className="flex flex-col items-center">
                  <div className="p-3 bg-white rounded-xl shadow-sm mb-4 group-hover:scale-110 transition-transform">
                    <item.icon className="w-6 h-6 text-primary" />
                  </div>
                  <h3 className="text-3xl font-bold mb-2 text-foreground bg-gradient-to-r from-primary to-primary/80 bg-clip-text text-transparent">
                    {item.metric}
                  </h3>
