  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-white overflow-hidden">
        ...
        <div className="fixed inset-0 overflow-hidden opacity-30 -z-10">
          <div className="grid grid-cols-6 md:grid-cols-12 gap-4 h-full w-full transform -rotate-12 scale-125 translate-x-1/4">
            {Array.from({ length: 240 }).map((_, i) => (
              <div key={i} className="bg-purple-100 h-full rounded-lg"></div>
            ))}
          </div>
        </div>
        ...
        <nav className="bg-white border-b border-gray-200 py-4 px-6 flex justify-between items-center">
          <Link to="/" className="flex items-center space-x-2">
            <img 
              ...
              className="h-8"
            />
            <span className="text-xl font-bold text-gray-800">PedagoIA</span>
          </Link>
          <div className="flex space-x-4">
            <Link to="/login" className="text-purple-600 hover:text-purple-800">
              Se connecter
            </Link>
            <Link to="/pricing" className="px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 transition-colors">
        ...
        {/* Main content */}
        <div className="max-w-4xl mx-auto space-y-8 p-6 py-12">
          <Card className="shadow-md border overflow-hidden relative bg-card/95 backdrop-blur-sm p-8">
            ...
            {/* Les autres sections conservent la même structure que l'original */}
            {/* ... sections générateur-sequences, exercices-evaluations, etc. */}
